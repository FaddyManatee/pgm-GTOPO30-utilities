...
echo -n Test 44: Reading a file with maximum gray value exceeding 255 fails
...
echo -n Test 45: Reading a file with a long comment fails
exeOut="$(./pgmEcho pgmImages/aFeepCommentLength.pgm output.pgm)"
expected="ERROR: Bad Comment Line (pgmImages/aFeepCommentLength.pgm)"
if [[ $exeOut =  $expected ]]; then
    printPassed
    passed=$((passed+1))
else
...
echo -n Test 46: pgmEcho correctly echoes an ascii file
exeOut="$(./pgmEcho pgmImages/x31_f18.ascii.pgm output.pgm)"
expected="ECHOED"
exeCompOut="$(./pgmComp pgmImages/x31_f18.ascii.pgm output.pgm)"
expectedComp="IDENTICAL"
if [[ $exeOut =  $expected ]]; then
    if [[ $exeCompOut =  $expectedComp ]]; then
        printPassed
        passed=$((passed+1))
    else
        printFailed
        failed=$((failed+1))
        assertionFailed "\${exeCompOut}" "\${expectedComp}"
    fi
else
    printFailed
    failed=$((failed+1))
...
echo -n Test 47: pgmEcho correctly echoes a binary file
exeOut="$(./pgmEcho pgmImages/sus.pgm output.pgm)"
expected="ECHOED"
exeCompOut="$(./pgmComp pgmImages/sus.pgm output.pgm)"
expectedComp="IDENTICAL"
if [[ $exeOut =  $expected ]]; then
    if [[ $exeCompOut =  $expectedComp ]]; then
        printPassed
...
echo -n Test 48: pgmComp correctly compares the same file
exeOut="$(./pgmComp pgmImages/sus.pgm pgmImages/sus.pgm)"
expected="IDENTICAL"
if [[ $exeOut =  $expected ]]; then
    printPassed
    passed=$((passed+1))
...
echo -n Test 49: pgmComp correctly compares different files
exeOut="$(./pgmComp pgmImages/sus.pgm pgmImages/columns.pgm)"
expected="DIFFERENT"
if [[ $exeOut =  $expected ]]; then
    printPassed
    passed=$((passed+1))
...
echo -n Test 50: pgmComp correctly compares the same file but \in different format
./pgmb2a pgmImages/sus.pgm output.pgm > /dev/null
exeOut="$(./pgmComp pgmImages/sus.pgm output.pgm)"
expected="IDENTICAL"
if [[ $exeOut =  $expected ]]; then
    printPassed
    passed=$((passed+1))
...
echo -n Test 51: pgma2b correctly converts an ascii file to its binary equivalent
exeOut="$(./pgma2b pgmImages/x31_f18.ascii.pgm output.pgm)"
expected="CONVERTED"
exeCompOut="$(./pgmComp pgmImages/x31_f18.ascii.pgm output.pgm)"
expectedComp="IDENTICAL"
if [[ $exeOut =  $expected ]]; then
...
echo -n Test 52: pgmb2a correctly converts a binary file to its ascii equivalent
exeOut="$(./pgmb2a pgmImages/sus.pgm output.pgm)"
expected="CONVERTED"
exeCompOut="$(./pgmComp pgmImages/sus.pgm output.pgm)"
expectedComp="IDENTICAL"
if [[ $exeOut =  $expected ]]; then
